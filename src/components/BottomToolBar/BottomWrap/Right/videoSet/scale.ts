...
class Scale extends Component {
  ...
  init() {
    let name = new Component(this.element, "div", { class: "scale__name" })
      .element;
    name.innerHTML = "视频比例";
    this.element.appendChild(name);
    this.select = new Component(this.element, "div", {
      class: "scale__select",
    }).element;
  ...
  appendchoselist() {
    ...
    list.forEach((item, index) => {
      let div = new Component(this.select, "div", {
        class: "scale__select__item",
      }).element;
      div.innerHTML = item;
      if (index == 0) {
