...
export class RightContainer extends Component {
  constructor(container: HTMLElement, video: Video, Player: Player) {
    ...
    if (volumne) new Volume(this.element, video, Player);
    if (rate) new Rate(this.element, video);
    if (setting) new VideoSet(this.element, video);
    if (screenShot) new ScreenShot(this.element, video);
    if (smallWindow && !video.Env) new SmallWindow(this.element, video, Player);
    if (webFullScreen && !video.Env)
      new WebFullScreen(this.element, video, Player);
    if (fullScreen) new FullScreen(this.element, video, Player);
  }
}
